...
A separate service, `infra/lambdas/review-pipeline/`, built around the orchestration problem underneath document/record review: deciding which patient record an uploaded document's extracted metadata (name, DOB, MRN) belongs to, when the extraction is noisy or the candidate pool has near-duplicates — and doing that through coordinated agent roles rather than one monolithic call.

**Multi-agent orchestration — supervisor/worker pattern, role-based delegation.** `supervisor_decompose` breaks the matching task into per-field subtasks and delegates each to a specialized worker node that owns exactly one piece of evidence (`name_matcher`, `dob_matcher`, `identifier_matcher`, in `pipeline/nodes.py`). Each worker is independently swappable — a worker's internals (deterministic scoring today) are opaque to the supervisor, which only ever consumes its (score, confidence) contract. `supervisor_aggregate` then reconciles all worker output into one ranked decision, the coordination point where role-based, delegated work gets reassembled into a single outcome.

**Confidence-gated action selection and escalation logic.** Every worker returns a result *and* a confidence, not just an answer. `route_after_aggregate` is a genuine conditional routing decision, not a fixed if/else: below a match-score floor the outcome is `no_match`; below a confidence threshold on an otherwise-plausible match the case routes to `needs_human_review` instead of being auto-resolved; only above both does the graph commit. This is the core discipline the JD calls out — the system is designed to *not* act past the point its own evidence supports, rather than letting an agent commit to a low-confidence guess silently.

**Protocol integration — agent-to-tool.** The same pipeline is exposed two ways: an MCP server (`mcp_server.py`) defining explicit, discoverable tools (`review_match`, `list_review_thresholds`) with typed schemas, and a Lambda Function URL handler (`lambda_function.py`). Both call the identical `pipeline.run_review()` — the invocation contract is the protocol, not a per-caller wrapper, so a new caller (another agent, a different service) integrates against one interface instead of bespoke glue per integration.

**Structured, per-stage tracing for failure attribution.** Every node appends a step to a structured trace (`pipeline/logging_utils.py`) — per-field scores, per-worker confidence, which node the final decision came from, and a `weakest_link` field naming the least-reliable piece of evidence behind the call. A wrong final answer traces back to the exact worker/field that caused it instead of requiring a re-derivation from raw output. This is diagnostic today (a human or a future controller reads the trace), not yet a closed loop where the pipeline acts on its own trace.
